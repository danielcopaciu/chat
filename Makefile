...
ci-deploy:
	ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $(TARGET_USER)@$(TARGET_SERVER) 'docker stop $(SERVICE) || true && docker rm $(SERVICE) || true && docker pull $(DOCKER_REPOSITORY):latest && docker run -d --restart unless-stopped --name $(SERVICE) -p 443:443 $(DOCKER_REPOSITORY):latest'
